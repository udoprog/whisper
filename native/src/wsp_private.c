...
)
{
    ...
    READ4((char *)&ai->spp, buf->spp);
    READ4((char *)&ai->count, buf->count);
} // __wsp_parse_archive
...
)
{
    ...
    READ4(buf->spp, (char *)&ai->spp);
    READ4(buf->count, (char *)&ai->count);
} // __wsp_dump_archive
...
)
{
    ...
    size_t offset = sizeof(wsp_metadata_b) + sizeof(wsp_archive_b) * index;

    if (w->io->read(w, offset, sizeof(wsp_archive_b), (void **)&buf, e) == WSP_ERROR) {
        return WSP_ERROR;
    }

    __wsp_parse_archive(buf, ai);

    ai->points_size = sizeof(wsp_point_t) * ai->count;
    ai->retention = ai->spp * ai->count;
...
wsp_return_t __wsp_valid_archive(wsp_archive_t *prev, wsp_archive_t *cur, wsp_error_t *e)
{
    // validate archive.
